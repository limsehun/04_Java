...
public class TryCatchService {
	 ...
	 *             
	 * - catch(예외 참조 변수){ }
	 * try에서 던져진 예외를 ()에서 작성된 참조변수로 잡아서 처리하는 구문
	 *</pre>
	 */
	public void test2() {
	...
	public void test4() {
		...
		System.out.println("int 범위 정수만 입력해줘");}
			...
			// 장점 : 여러 예외처리 구문을 작성하지 않고도 예외처리가 가능

			// 단점 : 발생하는 예외별 처리방법을 제시할 수 없다 
			System.out.println("상정하지 않은 예외발생");
		}

		// InputMismatchException에 빨간줄(컴파일 에러)이 발생하는 이유 : 
		// 앞에서 부모타입 참조변수를 이용해 자식타입 예외를 이미 처리했기 때문에
		// 도달 할 수 없는 코드(dead code)가 됨 부모코드(RuntimeException)보다 앞에쓰셈
//		catch(InputMismatchException e) {
//			System.out.println("int 범위 정수만 입력해줘");}
	 ...
	 * 
	 * 1) Checked Exception
	 * - 반드시 예외처리 구문을 작성해야되는 예외
	 * - 예외 처리 구문 미작성시 컴파일러 에러 발생(빨간줄)
	 * 
	 * 2) Unchecked Exception
	 * - 예외처리 구문 작성 여부가 필수가 아닌 선택하는 예외
	 * - 예외 처리 구문을 작성하지 않아도 컴파일 에러 발생 X
	 * - 대부분 Unchecked Exception은 if문으로 해결 가능
	 * 		(사용자, 이용자의 실수 방지)
	 * 
	 */
